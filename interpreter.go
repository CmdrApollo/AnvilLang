...
func (i *Interpreter) RunInstruction(node ParseNode) *IValue {
	...
	case "func_decl":
		...
		funcArgs := node.Children[1].Children
		returnType := i.RunInstruction(*node.Children[2]).Type
		...
		for _, a := range funcArgs {
			...
			if len(arg.Children) > 2 {
				params = append(params, IParam{Name: arg.Children[0].Data, Type: i.RunInstruction(*arg.Children[1]).Type, Default: i.RunInstruction(*arg.Children[2])})
			} else {
				params = append(params, IParam{Name: arg.Children[0].Data, Type: i.RunInstruction(*arg.Children[1]).Type})
			}
		}
		...
		break
	case "struct_decl":
		structName := node.Children[0].Data
		structFields := node.Children[1].Children
		...
		for _, a := range structFields {
			...
			if len(field.Children) > 2 {
				fields = append(fields, IParam{Name: field.Children[0].Data, Type: i.RunInstruction(*field.Children[1]).Type, Default: i.RunInstruction(*field.Children[2])})
			} else {
				fields = append(fields, IParam{Name: field.Children[0].Data, Type: i.RunInstruction(*field.Children[1]).Type})
			}
		}
	...
	case "return_stmt": // done
		...
		i.ReturnValue = i.RunInstruction(*node.Children[0])
		break
	...
	case "while_stmt": // done
		...
		break
	case "loop_stmt":
		values := i.RunInstruction(*node.Children[0])
		varName := node.Children[1].Data
		varType := i.RunInstruction(*node.Children[2]).Type
		suite := node.Children[3]
	...
	case "if_stmt": // done
		...
		if IsTruthy(i.RunInstruction(*node.Children[0]).Value) {
			i.Scopes = append(i.Scopes, make(map[string]IValue))
			i.CurrentScope = i.Scopes[len(i.Scopes)-1]
	...
	case "variable_decl": // done
		varName := node.Children[0].Data
		varType := i.RunInstruction(*node.Children[1])
		varValue := i.RunInstruction(*node.Children[2])
	...
	case "mut_stmt": // done
		...
		if varName.Name == "get_item" {
			value := i.RunInstruction(*varName.Children[0])
			index := i.RunInstruction(*varName.Children[1])
			t := value.Type
			if t.Type == "arr" {
				varIndex = index.Value.(int)
				varValue := i.RunInstruction(*node.Children[1])
			...
			structName := varName.Children[0].Data
		...
		break
	case "get_item":
		variable := i.RunInstruction(*node.Children[0])
		for j, child := range node.Children {
			if j == 0 {
				continue
			}
			expr := i.RunInstruction(*child)
			var array []any
			...
			if intValue < 0 || intValue >= len(array) {
				fmt.Println(node.Children[0].Data)
	...
	case "type": // done
		...
		typeNode.Type = i.RunInstruction(*node.Children[1]).Type

		if len(node.Children) > 2 {
			typeNode.Type.Subtype = i.RunInstruction(*node.Children[2]).Type
		}

		if len(node.Children) > 3 {
			typeNode.Type.Subtype = IType{Type: typeNode.Type.Subtype.(IType).Type, Subtype: i.RunInstruction(*node.Children[3]).Type}
		}

		typeNode.Type.Nullable = i.RunInstruction(*node.Children[0]).Value.(bool)

		return typeNode
	case "array":
		elements := make([]any, 0)
		for _, child := range node.Children {
			elements = append(elements, i.RunInstruction(*child))
		}
	...
	case "map":
		...
		for _, child := range node.Children {
			childKey := i.RunInstruction(*child.Children[0])
			childValue := i.RunInstruction(*child.Children[1])
	...
	case "func_call": // done
		...
		for j := 1; j < len(node.Children); j++ {
			arguments = append(arguments, i.RunInstruction(*node.Children[j]))
		}
	...
	case "muldiv_operation": // done
		a := i.RunInstruction(*node.Children[0])
		op := node.Children[1].Data
		b := i.RunInstruction(*node.Children[2])
		if op == "star" {
	...
	case "or_operation": // done
		a := i.RunInstruction(*node.Children[0]).Value.(bool)
		b := i.RunInstruction(*node.Children[1]).Value.(bool)
